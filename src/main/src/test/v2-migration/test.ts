...
function assertComponentGraphIsFlattened(project: V2.Data) {
    values(project.steps).forEach((step) => {
        ...
        assert.ok(
            !("elements" in step.payload),
            "Component.create payload still has inline elements"
        );
    });
...
function assertSyntheticMigration(project: V2.Data) {
    ...
    assert.equal(
        project.pluginPointers[component.introTransition.plugin!].name,
        "transition-plugin"
    );
...
function assertRealProjectMigration(input: V1.Data, project: V2.Data) {
    ...
    assert.ok(
        pluginElement.payload.plugin === null ||
            project.pluginPointers[pluginElement.payload.plugin]
    );
