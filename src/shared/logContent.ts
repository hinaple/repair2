  ...
  if (className === "Map")
    return new Map(
      (value as Map<any, any>)
        .entries()
        .map(([k, v]) => [normalizeSingleLogContent(k, _refs), normalizeSingleLogContent(v, _refs)])
    );
  if (className === "Set")
    return new Set((value as Set<any>).values().map((v) => normalizeSingleLogContent(v, _refs)));
