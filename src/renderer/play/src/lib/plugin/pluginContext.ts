...
function getApi<K extends keyof ContextApi, C extends ContextApiObject[K]>(
    ...
    if (typeof api === "function") {
        if (!pluginData)
            throw new Error(
                `"${key}" is dynamic plugin context api, but did not recevied plugin data.`
            );
        ...
        api = api({
            ...
            error: (content, error = null) =>
                reportPluginException(pluginData.plugin, content, error),
...
function createLogger(plugin: SDK.PluginIdentity): SDK.LoggerApi {
    return {
        debug: (...args: unknown[]) =>
            sendPluginLog({ level: "debug", source: plugin, content: args }),
        info: (...args: unknown[]) =>
            sendPluginLog({ level: "info", source: plugin, content: args }),
        warn: (...args: unknown[]) =>
            sendPluginLog({
                level: "warning",
                source: plugin,
