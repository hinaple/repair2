...
export default class Step extends TypePayload {
  title = $state();
  constructor({ id = genId(), type = null, title = null, payload = {} } = {}, creatingOpt = null) {
    if ((type?.join(".") ?? type) === "Communication.Socket.send" && !Array.isArray(payload.data)) {
      payload.data =
  ...
  get displayTitle() {
    ...
    if (this.type === "Others.runtimePluginStep" && this.payload.pluginName && this.payload.step)
