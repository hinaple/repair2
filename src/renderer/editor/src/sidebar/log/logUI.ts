...
function spanContent(
  ...
  if (type === "function") {
    ...
    const prefixLen = sv.startsWith("function") ? 0 : sv.startsWith("async") ? "async ".length : -1;
    return [
    ...
    preview.append(
      ...
      ...joinComma(
        [...content.entries()].map(([k, c]) =>
          newEl("span.pair", [createLogContent(k, 2).el, " => ", createLogContent(c, 2).el])
        )
      ),
      "}"
    );
...
function collapsableFullList(
  ...
  return lineArr.map((l) => {
    const lineEl = newEl(["div", "expanded-line"], [newEl("span.key", String(l.key)), ": ", l.el]);
