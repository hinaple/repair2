...
`title`은 runtime resource title로 resource를 선택합니다. `<repair-asset>`은 일치하는 resource를 내부 `<img>` 또는 `<video>` element로 렌더링합니다.
...
Function plugin은 `function` property가 있는 객체 또는 그 객체를 반환하는 factory를 export합니다.
...
Bare function export는 현재 contract에 포함되지 않습니다.
...
Keyframe을 반환하는 `function` property도 export할 수 있습니다. Function은 `{ attributes, ctx, ...argument }`를 받고 promise를 반환할 수 있습니다. 현재 런타임은 keyframe array를 반환하는 function도 허용합니다. Direct keyframe array default export는 현재 contract에 포함되지 않습니다.
...
| `element`    | mount 함수                                                                     |
| `frame`      | mount 함수                                                                     |
| `function`   | `function`을 포함하는 객체, 또는 그것을 반환하는 factory function              |
| `transition` | `keyframes` 또는 `function`을 가진 객체, 또는 그것을 반환하는 factory function |
| `runtime`    | 객체 또는 factory function                                                     |
